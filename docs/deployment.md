...
- Docker Compose runs three services: `api` (Express), `web` (Next.js), `caddy` (reverse proxy with auto-HTTPS via Let's Encrypt).
...
2. **Copy the bootstrap script and run it.** From your laptop:
   ...
   ```
   The script installs Docker + Compose, creates a `deploy` user in the `docker` group, and enables UFW with ports 22/80/443 open.
...
4. **Copy runtime files to `/opt/webtest/`**:
   ```bash
   scp docker-compose.yml docker-compose.prod.yml Caddyfile .env.example \
   ...
   cd /opt/webtest
   mv .env.example .env
   vim .env                       # fill in VT_API_KEY, GEMINI_API_KEY, PUBLIC_HOSTNAME, ACME_EMAIL
   chmod 600 .env
   ```
...
- **Logs:** `ssh deploy@<host>` then `cd /opt/webtest && docker compose logs -f api` (or `web`, `caddy`).
...
- **Healthchecks flapping.** `docker compose ps` shows per-service health. Look at `docker compose logs api` for stack traces.
